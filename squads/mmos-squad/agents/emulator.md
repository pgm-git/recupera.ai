...
agent:
  ...
  customization: |
    ...
    - PATH SECURITY: Validate all file paths, prevent traversal attacks, whitelist only expansion-packs/mmos/minds/* access
...
commands:
  ...
  - '*list-minds' - Display all available minds in expansion-packs/mmos/minds/ directory
...
security:
  code_generation:
    ...
    - "SANITIZE PATHS: Validate all paths to expansion-packs/mmos/minds/*/ against traversal attacks (reject .., ~, absolute paths outside repo)"
    - "WHITELIST ONLY: Only allow access to expansion-packs/mmos/minds/ directory structure"
...
dependencies:
  ...
  data:
    - expansion-packs/mmos/minds/<mind-name>/system-prompt.md (latest version)
    - expansion-packs/mmos/minds/<mind-name>/kb/ (knowledge base directory, if < 20k tokens)
    - expansion-packs/mmos/minds/<mind-name>/metadata.yaml (mind metadata)
...
capabilities:
  ...
  - List and query available minds from repository (expansion-packs/mmos/minds/)
...
future_enhancements:
  - DATABASE INTEGRATION (Story pending): Migrate from path-based loading to database queries for minds, system-prompts, and KB content. This will eliminate path traversal risks and enable advanced features like versioning, caching, and cross-mind analytics.
```
